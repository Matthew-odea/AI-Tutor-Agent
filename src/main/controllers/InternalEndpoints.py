...
def upload_file_context(
    ...
    """Upload a PDF, extract its text and index it as a context document."""
    _assert_instructor_access(principal)
    try:
...
async def get_upload_url(
    ...
    """Presigned S3 PUT URL (1 hour). The key is built server-side from the principal, so callers can only write under their own prefix:
    ...
    """
    # Any authenticated principal (students need this for audio answers), but an
    # assessment-scoped token may only write into its own assessment.
